...
@property (nonatomic, readwrite, retain) NSMutableArray *legendRects;
...
/** @property NSMutableArray *legendRects
 *  @brief An array of all legend rects.
 **/
@synthesize legendRects;
...
-(id)initWithFrame:(CGRect)newFrame
{
    if ( (self = [super initWithFrame:newFrame]) ) {
        ...
        legendRects           = [[NSMutableArray alloc] init];
        layoutChanged         = YES;
...
-(id)initWithLayer:(id)layer
{
    if ( (self = [super initWithLayer:layer]) ) {
        ...
        legendRects           = [theLayer->legendRects retain];
...
-(void)encodeWithCoder:(NSCoder *)coder
{
    ...
    [coder encodeObject:self.legendRects forKey:@"CPTLegend.legendRects"];
...
-(id)initWithCoder:(NSCoder *)coder
{
    if ( (self = [super initWithCoder:coder]) ) {
        ...
        legendRects           = [[coder decodeObjectForKey:@"CPTLegend.legendRects"] mutableCopy];
...
-(void)renderAsVectorInContext:(CGContextRef)context
{
    ...
    for ( CPTLegendEntry *legendEntry in self.legendEntries ) {
        ...
        if ( ( (desiredRowCount == 0) || (row < desiredRowCount) ) &&
            ...
            NSValue *aValue = [NSValue valueWithCGRect:CPTAlignRectToUserSpace( context, CGRectMake(columnPositions[col], rowPosition, actualColumnWidths[col] + theSwatchSize.width + theOffset, actualRowHeights[row]) )];
            if (![self.legendRects containsObject:aValue]) {
                [self.legendRects addObject:aValue];
            }
        }
    }
...
//        }
    ...
    CPTLegendEntry *theSelectedLegendEntry = nil;
    CGRect theRect = CGRectZero;
        
        CGPoint pointInPlotArea = [self.graph convertPoint:interactionPoint toLayer:self];
        if ( [self containsPoint:pointInPlotArea] ) {
            
            for (NSUInteger i = 0; i < [self.legendRects count]; i++) {
                NSLog(@"frame = %@", [self.legendRects objectAtIndex:i]);
                NSLog(@"pointInPlotArea = %f , %f", pointInPlotArea.x, pointInPlotArea.y);
                CGRect aFrame = [[self.legendRects objectAtIndex:i] CGRectValue];
                if (CGRectContainsPoint(aFrame, pointInPlotArea)) {
                    NSLog(@"Point is in rect");
                    if ([self.legendEntries count] > i) {
                        theSelectedLegendEntry = [self.legendEntries objectAtIndex:i];
                        theRect = aFrame;
                    }
                }
            }
            // Handle event
            //TODO:call touch event
            NSLog(@"Legend is touched");
            
            if (theSelectedLegendEntry) {
                
                id<CPTLegendDelegate> theDelegate = self.delegate;
                if ( [theDelegate respondsToSelector:@selector(legend: tappedOnLegendEntry: atIndex: forPlot: inRect:)] ) {
                    [theDelegate legend:self tappedOnLegendEntry:theSelectedLegendEntry atIndex:theSelectedLegendEntry.index forPlot:theSelectedLegendEntry.plot inRect:theRect];
                }
            }
